...
import { getBrazilianPhoneDisplay, getBrazilianWhatsAppHref } from '@/lib/utils';
...
export function StoreFooter({
  ...
  const whatsappPhone = settings.catalog_whatsapp || '';
  const whatsapp = getBrazilianWhatsAppHref(whatsappPhone);
  const whatsappDisplay = getBrazilianPhoneDisplay(whatsappPhone);
  const commercialPhone = company.phone || settings.top_bar_phone || '';
  ...
  const ordersHref = storeCustomerAuthenticated
    ? STORE_ROUTES.orders
    : withStoreRedirect(STORE_ROUTES.login, STORE_ROUTES.orders);
    ...
    >
      ...
      {showFloatingWhatsApp && whatsapp && (
        <a
          ...
          className="fixed bottom-[calc(6.15rem+env(safe-area-inset-bottom))] left-3 z-40 inline-flex max-w-[calc(100vw-5.75rem)] items-center gap-2.5 rounded-2xl bg-[#25D366] px-3.5 py-3 text-white shadow-lg shadow-emerald-950/20 transition-all hover:-translate-y-0.5 hover:bg-[#20ba5a] hover:shadow-xl active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#25D366]/50 focus-visible:ring-offset-2 md:bottom-6 md:left-6 md:max-w-none md:px-4"
          title="Fale Conosco no WhatsApp"
          aria-label="Fale conosco pelo WhatsApp"
        >
          <svg className="h-5 w-5 shrink-0 fill-current md:h-6 md:w-6" viewBox="0 0 24 24" aria-hidden="true">
            <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946C.06 5.348 5.397.01 12.008.01c3.202.001 6.212 1.246 8.477 3.514 2.266 2.268 3.507 5.28 3.505 8.484-.004 6.657-5.34 11.997-11.953 11.997-2.005-.001-3.973-.502-5.724-1.457L0 24zm6.59-4.846c1.6.95 3.188 1.449 4.825 1.451 5.436 0 9.86-4.42 9.863-9.864.001-2.63-1.023-5.101-2.883-6.963C16.588 1.843 14.116.822 11.5.822 6.066.822 1.641 5.242 1.638 10.682c-.001 1.666.436 3.292 1.267 4.724L1.878 20.1l4.769-1.25zM17.51 14.86c-.3-.149-1.772-.875-2.046-.975-.276-.1-.476-.149-.676.15-.2.3-.777.975-.951 1.174-.176.2-.351.224-.651.075-.3-.149-1.268-.467-2.417-1.493-.892-.796-1.495-1.78-1.67-2.079-.176-.3-.019-.462.13-.611.134-.133.3-.35.45-.525.15-.175.2-.299.3-.5.1-.2.05-.375-.025-.525-.075-.15-.676-1.625-.926-2.225-.244-.582-.491-.504-.676-.513-.175-.008-.375-.01-.575-.01-.2 0-.525.075-.8.375-.276.3-1.05 1.025-1.05 2.5s1.075 2.9 1.225 3.1c.15.2 2.11 3.224 5.112 4.521.714.309 1.272.494 1.707.632.716.227 1.368.195 1.884.118.574-.085 1.772-.724 2.022-1.424.25-.7.25-1.299.175-1.424-.075-.125-.275-.199-.575-.349z"/>
          </svg>
          <span className="flex min-w-0 flex-col text-left leading-tight">
            <span className="text-[11px] font-black uppercase tracking-wide md:text-xs">FALE CONOSCO</span>
            <span className="text-[8px] font-extrabold uppercase tracking-wide opacity-90 md:text-[9px]">CLIQUE AQUI PARA COMEÇAR</span>
          </span>
        </a>
      )}
      ...
      >
        ...
        <span className="hidden text-xs font-black uppercase tracking-wide sm:inline">Topo</span>
      </button>
    </footer>
  );
}
