...
import { buildWhatsAppUrl, renderConfiguredWhatsAppTemplate, validateWhatsAppTemplate, WHATSAPP_TEMPLATE_MAX_LENGTH } from '@/lib/whatsapp';
...
export default function WhatsAppCenterPage() {
  ...
  const preview = useMemo(
    () => renderConfiguredWhatsAppTemplate(content, selected.definition, selected.definition.sampleVariables, settings || undefined),
    [content, selected.definition, settings]
  );
    ...
    <div className="space-y-5">
      ...
      {tab === 'templates' ? (
        <div className="grid min-w-0 gap-4 xl:grid-cols-[270px_minmax(0,1fr)_320px]">
          ...
          <section className="min-w-0 rounded-2xl border border-border bg-card p-4">
            ...
            <div className="mt-4"><div className="mb-2 flex items-center gap-2 text-[11px] font-bold text-foreground"><Variable className="h-4 w-4" />Variáveis disponíveis</div><div className="flex flex-wrap gap-2">{selected.definition.allowedVariables.map((variable) => <button key={variable} type="button" onClick={() => insertVariable(variable)} className="min-h-9 rounded-lg border border-primary/20 bg-primary/5 px-2.5 font-mono text-[10px] text-primary hover:bg-primary/10">{`{{${variable}}}`}</button>)}</div></div>
            <div className="mt-5 flex flex-wrap justify-end gap-2"><button type="button" onClick={handleRestore} disabled={saving || !selected.custom} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-border px-3 text-xs font-bold disabled:opacity-40"><RotateCcw className="h-4 w-4" />Restaurar padrão</button><button type="button" onClick={handleSaveTemplate} disabled={saving || !validation.valid || !dirty} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-primary px-4 text-xs font-bold text-primary-foreground disabled:opacity-40"><Save className="h-4 w-4" />{saving ? 'Salvando...' : 'Salvar modelo'}</button></div>
          </section>

          <aside className="min-w-0 rounded-2xl border border-border bg-card p-4">
            <div className="flex items-center justify-between"><h2 className="text-xs font-black text-foreground">Pré-visualização</h2><button type="button" onClick={() => void navigator.clipboard.writeText(preview)} className="inline-flex min-h-9 items-center gap-1 rounded-lg border border-border px-2 text-[10px] font-bold"><Clipboard className="h-3.5 w-3.5" />Copiar</button></div>
      ...
      {testOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby="test-whatsapp-title">
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card p-5 shadow-2xl"><div className="flex items-center justify-between"><h2 id="test-whatsapp-title" className="text-sm font-black">Testar mensagem</h2><button type="button" onClick={() => setTestOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-xl hover:bg-secondary" aria-label="Fechar"><X className="h-4 w-4" /></button></div><p className="mt-1 text-[11px] text-muted-foreground">Nenhuma mensagem será enviada automaticamente.</p><label className="mt-4 block text-xs font-bold">Número de teste<input value={testPhone} onChange={(event) => setTestPhone(event.target.value)} placeholder="(51) 99999-9999" className="mt-1 h-11 w-full rounded-xl border border-border bg-background px-3 text-xs outline-none focus:border-primary" /></label><div className="mt-3 max-h-56 overflow-y-auto whitespace-pre-wrap rounded-xl bg-secondary/50 p-3 text-[11px] leading-5">{preview}</div><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setTestOpen(false)} className="min-h-10 rounded-xl border border-border px-4 text-xs font-bold">Cancelar</button><a href={testUrl || undefined} target="_blank" rel="noopener noreferrer" aria-disabled={!testUrl} onClick={(event) => { if (!testUrl) event.preventDefault(); else setTestOpen(false); }} className={`inline-flex min-h-10 items-center gap-2 rounded-xl bg-emerald-600 px-4 text-xs font-bold text-white ${!testUrl ? 'pointer-events-none opacity-40' : 'hover:bg-emerald-700'}`}><Send className="h-4 w-4" />Confirmar e abrir</a></div></div>
        </div>
      )}
    </div>
  );
...
function TabButton({ active, onClick, icon, children }: { active: boolean; onClick: () => void; icon: React.ReactNode; children: React.ReactNode }) {
  return <button type="button" onClick={onClick} className={`inline-flex min-h-10 flex-1 items-center justify-center gap-2 rounded-lg px-3 text-xs font-bold transition sm:flex-none ${active ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-secondary'}`}>{icon}{children}</button>;
}
...
function ConfirmDialog({ title, description, confirmLabel, onCancel, onConfirm }: { title: string; description: string; confirmLabel: string; onCancel: () => void; onConfirm: () => void }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-labelledby="confirm-title"><div className="w-full max-w-sm rounded-2xl border border-border bg-card p-5 shadow-2xl"><h2 id="confirm-title" className="text-sm font-black">{title}</h2><p className="mt-2 text-xs leading-5 text-muted-foreground">{description}</p><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onCancel} className="min-h-10 rounded-xl border border-border px-4 text-xs font-bold">Continuar editando</button><button type="button" onClick={onConfirm} className="min-h-10 rounded-xl bg-rose-600 px-4 text-xs font-bold text-white">{confirmLabel}</button></div></div></div>;
}
