...
import {
  authorizeOperationalSystemMessageContext,
...
type OperationalEventKey = 'quote_proposal' | 'order_payment_pending' | 'production_status_changed';
...
function parseRequestBody(value: unknown): {
  ...
  if (keys.some((key) => !['eventKey', 'contextId'].includes(key))) return null;
  if (typeof body.eventKey !== 'string' || !OPERATIONAL_EVENTS.has(body.eventKey)) return null;
  if (typeof body.contextId !== 'string' || !body.contextId.trim() || body.contextId.length > 128) return null;
  return {
    eventKey: body.eventKey as OperationalEventKey,
    contextId: body.contextId.trim()
  };
}

function buildContext(eventKey: OperationalEventKey, contextId: string): WhatsAppSystemMessageContext {
...
function friendlyResolutionError(error: unknown) {
  ...
  if (message.endsWith('_PIX_NOT_CONFIGURED')) {
    return ['Configure a chave PIX em Configurações → Finanças & Chave PIX antes de enviar esta cobrança.', 422] as const;
  }
  if (message.endsWith('_ORDER_NOT_PAYABLE')) return ['Este pedido já está totalmente quitado.', 422] as const;
  if (message.endsWith('_RECIPIENT_MISSING')) {
    return ['Telefone/WhatsApp do destinatário não encontrado. Atualize o cadastro antes de continuar.', 422] as const;
  }
  ...
  return ['Não foi possível preparar a mensagem agora.', 500] as const;
}
...
export async function POST(request: Request) {
  try {
    ...
    const { trustedCompanyId } = await authorizeOperationalSystemMessageContext(request, body.eventKey);
    ...
    return noStoreJson({
      ...
      confirmBeforeOpen: resolved.confirmBeforeOpen,
      href: resolved.active ? resolved.testHref : ''
    });
  } catch (error) {
