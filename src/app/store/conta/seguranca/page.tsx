...
export default function StoreAccountSecurityPage() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!user?.email) {
      setError('Sessao invalida. Entre novamente para alterar a senha.');
      return;
    }

    if (newPassword.length < 8 || !/[a-zA-Z]/.test(newPassword) || !/\d/.test(newPassword)) {
      setError('A nova senha precisa ter pelo menos 8 caracteres, com letras e numeros.');
      return;
    }
    ...
    try {
      ...
      setError(securityError instanceof Error ? securityError.message : 'Nao foi possivel alterar a senha.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <StoreAccountShell title="Seguranca e senha" subtitle="Altere sua senha de acesso com validacao da senha atual.">
      <form onSubmit={submit} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
          ...
          <p className="mt-2 text-xs font-semibold leading-5 text-slate-500">
            A senha e atualizada diretamente no Supabase Auth. Ela nao e salva em tabelas proprias do sistema.
          </p>
        </div>
