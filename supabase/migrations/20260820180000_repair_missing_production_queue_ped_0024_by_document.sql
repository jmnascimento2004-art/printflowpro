-- Second targeted attempt for the single production gap confirmed by the
-- read-only Production/Orders inventory on 2026-08-20. The tenant is matched
-- by its canonical CNPJ because the previous display-name match was a safe
-- no-op. No existing queue row or operational stage is modified.
...
begin
  ...
  if v_candidate_count = 0 then
    raise notice 'ORDER_PRODUCTION_REPAIR_TARGET_NOT_PRESENT_BY_DOCUMENT';
    return;
  end if;
  ...
  if v_candidate_count <> 1 then
    raise exception using
      errcode = 'P0001',
      message = 'ORDER_PRODUCTION_REPAIR_TARGET_AMBIGUOUS_BY_DOCUMENT';
  end if;
  ...
  if v_existing_count = v_item_count then
    raise notice 'ORDER_PRODUCTION_REPAIR_ALREADY_COMPLETE_BY_DOCUMENT';
    return;
  end if;
  ...
  insert into public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'source', 'targeted_idempotent_repair_20260820180000_by_document'
    )
  );
end;
$$;
