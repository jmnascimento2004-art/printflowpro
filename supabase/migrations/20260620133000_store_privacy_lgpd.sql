...
begin
  ...
  values
    (p_company_id, resolved_customer_id, current_auth_user, 'privacy_policy', true, p_privacy_policy_version, 'store_signup', now(), null),
    (p_company_id, resolved_customer_id, current_auth_user, 'terms_of_use', true, p_terms_version, 'store_signup', now(), null),
    (p_company_id, resolved_customer_id, current_auth_user, 'marketing_email', coalesce(p_marketing_email_granted, false), p_privacy_policy_version, 'store_signup', case when coalesce(p_marketing_email_granted, false) then now() else null end, case when coalesce(p_marketing_email_granted, false) then null else now() end),
    (p_company_id, resolved_customer_id, current_auth_user, 'marketing_whatsapp', coalesce(p_marketing_whatsapp_granted, false), p_privacy_policy_version, 'store_signup', case when coalesce(p_marketing_whatsapp_granted, false) then now() else null end, case when coalesce(p_marketing_whatsapp_granted, false) then null else now() end);
  ...
  values (
    ...
    jsonb_build_object('privacy_policy_version', p_privacy_policy_version, 'terms_version', p_terms_version),
    'store_signup'
  );
